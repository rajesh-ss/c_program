 ...
 * @file Q1_array.c
 * @author Rajesh S
 * @brief Generate a Structure of reg, name and per_sc 'Using ARRAY'
 * @version 0.1
 * @date 2022-04-30
 *
...
int get_arr_usr(std arr[])
{
    ...
    for (i = 0; i < arr_size; i += 1)
    {
        ...
        printf("\nEnter your name\t");
        fgets(arr[i].name, gen_len, stdin);
...
void dis_arr(std *arr, int arr_size){
   ...
   // printf("\n_______________________________________________________\n");
    ...
    for(i=0; i<arr_size; i+=1){
        //printf("\t%*s___%*s___%*s\n", -0, und[0], -0, und[1], 0, und[2]);
        printf("\n\t%*d | %*s | %*d\n", -3, arr[i].reg, -MAX, arr[i].name, MAX, arr[i].per_sc);
        //printf("\t%*s___%*s___%*s\n", -0, und[0], -0, und[1], 0, und[2]);
    }
